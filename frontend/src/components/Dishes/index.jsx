  ...
  return (
    <section className="dishes-container mt-10 py-8">
      <div className="main__content">
        ...
        <div className="  flex justify-center flex-wrap gap-7 w-full  mt-14">
          {items.map(({ image, title, price, description }, index) => (
            <div
              key={index}
              className="w-[260px] shadow-lg relative top-0 mt-5 rounded-md"
            >
              <img
                src={image}
                alt={title}
                className="h-52 w-full object-center  rounded-t-md"
              />
